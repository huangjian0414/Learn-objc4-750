...
#   define POOL_BOUNDARY nil
    ...
    magic_t const magic;// 魔数，用于自身的完整性校验  16
    id *next;// 指向autorelePool page中的下一个可用位置。8
    pthread_t const thread;// 和autorelePool page中相关的线程。8
    AutoreleasePoolPage * const parent;// autoreleasPool page双向链表的前向指针。8
    AutoreleasePoolPage *child;// autoreleasPool page双向链表的后向指针。8
    uint32_t const depth;// 当前autoreleasPool page在双向链表中的位置（深度）4
    uint32_t hiwat;// high water mark. 最高水位，可用近似理解为autoreleasPool page双向链表中的元素个数       4
...
///将一个id放入到autoreleasePage
    static inline id *autoreleaseFast(id obj)
    {
        ///首先取出当前的hotPage，所谓hotPage，就是在autoreleasePage链表中正在使用的autoreleasePage节点
        AutoreleasePoolPage *page = hotPage();
        if (page && !page->full()) {//如果有hotPage，且hotPage还没满，这将obj加入到page中
            return page->add(obj);
        } else if (page) {//如果有hotPage，但是已经满了，则进入page full逻辑（autoreleaseFullPage）
            return autoreleaseFullPage(obj, page);
        } else {//如果没有hotPage，进入no page逻辑autoreleaseNoPage
...
public:
    ///将某个特定的对象指针插入到AutoreleasePoolPage
    static inline id autorelease(id obj)
    {
        assert(obj);
        assert(!obj->isTaggedPointer());//由于tagged pointer不遵循引用计数规则，所以也不会有autorelease操作。
        id *dest __unused = autoreleaseFast(obj);
        assert(!dest  ||  dest == EMPTY_POOL_PLACEHOLDER  ||  *dest == obj);
        return obj;
    }

///创建或获取当前的hotPage，并向对象栈中插入一个POOL_BOUNDARY
...
objc_object::sidetable_getExtraRC_nolock()
{
    ...
    else return it->second >> SIDE_TABLE_RC_SHIFT;//在返回引用计数前，还做了个右移2位的位操作，因为在sidetable中，引用计数的低2位不是用来记录引用次数的，而是分别表示对象是否有弱引用计数，以及是否在deallocing
}
...
callAlloc(Class cls, bool checkNil, bool allocWithZone=false)
{
    ...
    /*
     告诉编译器，if中的条件是大概率(fastpath)还是小概率(slowpath)事件，从而让编译器对代码进行优化
     **/
#if __OBJC2__
    if (fastpath(!cls->ISA()->hasCustomAWZ())) {//当前class是否有默认的allocWithZone，没有就走if里
