...
struct weak_entry_t {
    ...
    union {
        struct {
            ///可以通过弱引用该对象的对象指针的指针做hash（w_hash_pointer(objc_object **) & (entry->mask)），直接找到对象指针的指针在referrers中对应的weak_referrer_t *。
 ...
 */
struct weak_table_t {
    ///可以通过对象地址做hash（hash_pointer(objc_object *) & weak_table->mask），直接找到weak_entries中该对象对应的weak_entry_t
