...
objc_object::getIsa() 
{
    if (!isTaggedPointer()) return ISA();/// 不是taggedPointer 返回isa
...
{
    /// 最高4位  最高位 1 表示是TaggedPointer 后3为表示类型。objc-internal.h中可以找到枚举
...
objc_object::rootRetain(bool tryRetain, bool handleOverflow)
{
    ...
    if (slowpath(transcribeToSideTable)) {
        ...
        sidetable_addExtraRC_nolock(RC_HALF);// #define RC_HALF  (1ULL<<18)  溢出时 extra_rc 为1后面19个0 ， 除以2 相当于 右移18位
    }
...
{
    //如果是tagged pointer，则直接返回this，因为tagged pointer是不需要引用计数的
    if (isTaggedPointer()) return (uintptr_t)this;

    sidetable_lock();// 将objcet对应的sidetable上锁
    isa_t bits = LoadExclusive(&isa.bits);
    ClearExclusive(&isa.bits);
    //如果采用了优化的isa指针
    if (bits.nonpointer) {
        uintptr_t rc = 1 + bits.extra_rc;// 先读取isa.extra_rc extra_rc中存储的是引用计数减一，所以这里要+1
